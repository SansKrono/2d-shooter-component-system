...
func _inject_copy_button() -> void:
	...
	_copy_button.tooltip_text = "Copy all error/warning messages to the clipboard"
	...
	print("[CopyAllErrors] The plugin is ready, and the 'Copy All' button has been injected into the error panel.")
...
func _on_copy_all_pressed() -> void:
	if not is_instance_valid(_error_tree):
		push_warning("[CopyAllErrors] The error list control is invalid.")
		return
	...
	print("[CopyAllErrors] %d messages have been copied to the clipboard" % count)
	_flash_button("%d logs have been copied!" % count)
